...
@WebServlet("/CardCallbackServlet")
public class CardCallbackServlet extends HttpServlet {
    ...
    private void markOrderPaid(int orderId) {
        try (Connection conn = DriverManager.getConnection(
            ...
            PreparedStatement ps = conn.prepareStatement(
                "UPDATE orders SET status='Paid', payment_method='Card' WHERE order_id=?"
            );
            ps.setInt(1, orderId);
            ps.executeUpdate();
    ...
    // ✅ Send confirmation email with tracking code & total amount
