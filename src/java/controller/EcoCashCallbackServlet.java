...
@WebServlet("/EcoCashCallbackServlet")
public class EcoCashCallbackServlet extends HttpServlet {
    ...
    private void markOrderPaid(int orderId) {
        try (Connection conn = DriverManager.getConnection(
            ...
            PreparedStatement ps = conn.prepareStatement(
                "UPDATE orders SET status='Paid', payment_method='EcoCash' WHERE order_id=?"
            );
            ps.setInt(1, orderId);
            ps.executeUpdate();
